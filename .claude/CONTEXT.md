...
**The core loop (the handback contract, v0.15.0):** `list_my_queue` (my actionable work — `{ work, review }` sections since v0.17.0) + `get_mentions` (what was I asked?) → `get_card` (read the spec) → `update_card` (take it: move to the `in_progress`-role column) + `add_comment` (a one-line "on it") → do the work → `update_card` (→ the `review`-role column, **set `reviewerUserId` to the requester** — default the card's creator) + `add_comment` (a `handoff` on the timeline, `@`-mentioning the requester) → `mark_mentions_read`. Move to `done` **only when the human says so**; if stuck, move to `blocked` + a comment saying why. Resolve target columns **by role** (from `get_project`), never by hardcoded name. A card's status **is** its column; report results on the **timeline**, not by editing the description. Full agent playbook: the `using_kbrelay` skill (`.claude/skills/USING_KBRELAY.md`).
