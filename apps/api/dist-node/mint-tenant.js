...
async function hashPassword(password2) {
  ...
  const derived = await pbkdf2(password2, salt, ITERATIONS, KEY_BYTES);
  return `${ALGO}$${ITERATIONS}$${b64encode(salt)}$${b64encode(derived)}`;
}
var PASSWORD_ALGO = ALGO;
var bs = (x) => x;
async function pbkdf2(password2, salt, iterations, length) {
  const key = await crypto.subtle.importKey(
    "raw",
    bs(new TextEncoder().encode(password2)),
    "PBKDF2",
    false,
    ["deriveBits"]
  );
...
async function registerTenant(env2, input) {
  ...
  const now = Date.now();
  const tenantId2 = newId("t");
  const userId2 = newId("u");
  const agentId = newId("u");
  const slug = await uniqueTenantSlug(env2, input.tenantName);
  const passwordHash = await hashPassword(input.password);
  const humanHandle = await uniqueHandle(env2, tenantId2, deriveHandle(input.name));
  const agentHandle = await uniqueHandle(env2, tenantId2, "assistant", [humanHandle]);
  await env2.db.batch([
    ...
    ).bind(userId2, tenantId2, input.name, humanHandle, input.email, passwordHash, PASSWORD_ALGO, now, now),
    ...
    ).bind(agentId, tenantId2, agentHandle, now),
    env2.db.prepare(
      `INSERT INTO memberships (id, tenant_id, user_id, role, created_at) VALUES (?, ?, ?, 'admin', ?)`
    ).bind(newId("m"), tenantId2, userId2, now),
    env2.db.prepare(
      `INSERT INTO memberships (id, tenant_id, user_id, role, created_at) VALUES (?, ?, ?, 'member', ?)`
    ).bind(newId("m"), tenantId2, agentId, now)
  ]);
  return { tenantId: tenantId2, userId: userId2 };
}
async function createToken(env2, tenantId2, userId2, label2) {
  const secret2 = randomToken();
  const tokenHash = await sha256Hex(secret2);
  const id = newId("tok");
  const now = Date.now();
  await env2.db.prepare(
    `INSERT INTO api_tokens (id, tenant_id, user_id, token_hash, label, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(id, tenantId2, userId2, tokenHash, label2, now).run();
  return {
    token: { id, label: label2, createdAt: now, lastUsedAt: null },
    secret: secret2
  };
}
...
async function uniqueTenantSlug(env2, name3) {
  const base = slugify(name3);
  let attempt = base;
  for (let n = 2; n < 100; n++) {
...
function deriveHandle(name3) {
  const first = name3.toLowerCase().replace(/[^a-z0-9\s-]+/g, "").trim().split(/\s+/)[0];
  const cleaned = (first ?? "").replace(/-+/g, "-").replace(/^-+|-+$/g, "").slice(0, 30);
  return cleaned || "user";
}
async function uniqueHandle(env2, tenantId2, base, taken = []) {
  let attempt = base;
  for (let n = 2; n < 100; n++) {
    const clash = taken.includes(attempt) || await env2.db.prepare("SELECT 1 AS ok FROM users WHERE tenant_id = ? AND handle = ?").bind(tenantId2, attempt).first();
    if (!clash) return attempt;
    attempt = `${base}${n}`;
  }
...
// scripts/mint-tenant.ts
var tenantName = process.env.TENANT ?? process.env.NAME;
var email = process.env.EMAIL;
if (!tenantName || !email) {
  console.error('Usage: TENANT="Workspace" EMAIL=you@example.com [PASSWORD=\u2026] [NAME="You"] [LABEL=admin-key] node mint-tenant.js');
  process.exit(1);
}
var generatedPassword = !process.env.PASSWORD;
var password = process.env.PASSWORD ?? randomBytes(12).toString("base64url");
var name2 = process.env.OWNER_NAME ?? "Admin";
var label = process.env.LABEL ?? "admin-key";
var { env } = buildNodeBindings(process.env);
var { tenantId, userId } = await registerTenant(env, { email, password, name: name2, tenantName });
var { secret } = await createToken(env, tenantId, userId, label);
console.log("");
console.log("  \u2705 Tenant created (store the token \u2014 it is not recoverable):");
console.log("");
console.log(`     workspace : ${tenantName}`);
console.log(`     admin     : ${name2} <${email}>`);
