...
/**
 * Project-label CRUD (KBR-84). The label definitions are tenant-scoped (any
 * member manages them; the repo filters by tenant_id, so cross-tenant is
 * impossible). Attaching labels to a project is project-scoped — the router's
 * `access` scope on the PUT route enforces caller access to that project.
 */
